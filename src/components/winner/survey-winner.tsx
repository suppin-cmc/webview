...
import { setSurveyWinners } from '@/store/survey-winner';
...
export const SurveyWinner = ({ surveyId, survey }: SurveyWinnerProps) => {
  ...
  const form = useForm<WinnerFormType>({
    ...
    defaultValues: {
      winnerCount: '',
      eventPeriod: {
        startDate: formattDate(new Date(survey.startDate)) || '',
        endDate: formattDate(new Date(survey.endDate)) || '',
      },
      minLength: '',
      keywords: [],
    },
  ...
  const submitHandler = async (values: WinnerFormType) => {
    ...
    const { data } = await draftWinnersMutation(payload);

    dispatch(setSurveyWinners(data));

    router('/result/15?survey=7&type=winner&progress=done');
  };
